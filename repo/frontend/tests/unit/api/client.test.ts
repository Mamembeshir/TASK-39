...
describe('api client.request', () => {
  ...
  it('does not trigger unauthorized hook for non-401 errors', async () => {
    const unauthorizedSpy = vi.fn();
    setOnUnauthorized(unauthorizedSpy);
    ...
    await expect(client.request({ method: 'GET', path: '/api/orders' })).rejects.toMatchObject({ code: 'FORBIDDEN' });
    await Promise.resolve();
    expect(unauthorizedSpy).not.toHaveBeenCalled();
  });

  it('does not fire unauthorized handler a second time while one is already in flight', async () => {
    const unauthorizedSpy = vi.fn();
    setOnUnauthorized(unauthorizedSpy);
    ...
    // Three simultaneous 401s — handler should only fire once
    await Promise.allSettled([
      client.request({ method: 'GET', path: '/api/orders' }),
      client.request({ method: 'GET', path: '/api/favorites' }),
      client.request({ method: 'GET', path: '/api/tickets' }),
    ]);

...
describe('ticketsApi', () => {
  ...
  it('uploadTicketAttachments returns empty mediaIds when media field is absent', async () => {
    requestMock.mockResolvedValue({});
    const result = await uploadTicketAttachments([new File(['x'], 'x.png', { type: 'image/png' })]);
    expect(result).toEqual({ mediaIds: [] });
  });

...
const { createMediaService } = require("../../../../src/services/media/mediaService");
  ...
  const service = createMediaService({
    ...
    mediaRepository: {
      findMediaById: async () => ({
        ...
        ownerIds: [{ toString: () => "65f000000000000000000001" }],
        ownerRefs: { "65f000000000000000000001": 1 },
      }),
...
test("deleteMediaById allows administrators to delete media they do not own", async () => {
  let deletedId = null;
  const service = createMediaService({
    ALLOWED_MEDIA_MIME: {},
    createError,
  ...
  const result = await service.deleteMediaById({
    auth: { sub: "65f000000000000000000099", roles: ["administrator"] },
    mediaId: "media-1",
  });

  assert.equal(result.status, 200);
  assert.equal(deletedId, "media-1");
});
  ...
  const service = createMediaService({
    ...
    mediaRepository: {
      findMediaById: async () => ({
        ...
        createdBy: { toString: () => "65f000000000000000000001" },
        ownerIds: [{ toString: () => "65f000000000000000000001" }],
        ownerRefs: { "65f000000000000000000001": 2 },
        refCount: 3,
        storagePath: "file.png",
      }),
      findMediaReferences: async () => ({ reviewRef: null, ticketRef: null, contentRef: null }),
...
test("deleteMediaById denies creator after their stake is cleaned up (IDOR regression)", async () => {
  // Simulate state after creator (A) has already decremented and cleanupMediaOwnerRef
  // has removed them from ownerIds and ownerRefs, but createdBy still points to A.
  // B still holds a valid stake (refCount=1, ownerIds=[B]).
  const state = {
    _id: "media-1",
    createdBy: { toString: () => "65f000000000000000000001" }, // A — stake already revoked
    ownerIds: [{ toString: () => "65f000000000000000000002" }], // only B remains
    ownerRefs: { "65f000000000000000000002": 1 },
    refCount: 1,
    storagePath: "private/abc.jpg",
  };
  ...
  const service = createMediaService({
    ...
    mediaRepository: {
      findMediaById: async () => state,
      findMediaReferences: async () => ({ reviewRef: null, ticketRef: null, contentRef: null }),
      decrementMediaRefCount: async () => {
        throw new Error("should not decrement — creator's stake is gone");
      },
      deleteMediaById: async () => {
        throw new Error("should not hard-delete — B still owns this media");
      },
    },
    MEDIA_ENABLE_PROCESSING: false,
    MEDIA_UPLOAD_DIR: "/tmp/uploads",
  ...
  await assert.rejects(
    () =>
      service.deleteMediaById({
        auth: { sub: "65f000000000000000000001", roles: ["customer"] }, // creator A
        mediaId: "media-1",
      }),
    (error) => error && error.code === "MEDIA_NOT_FOUND" && error.status === 404,
  );
